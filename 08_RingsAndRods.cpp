...
// the rod labeled 3, a green ring placed onto the rod labeled 2, and a
// blue ring placed onto the rod labeled 1.
...
public:
    ...
    {
        int count = 0;
        vector<set<char>> p(10);
        for (int i = 1; i < rings.length(); ++i)
        {
            p[rings[i] - 48].insert(rings[i - 1]);
            ++i;
        }
        for (int i = 0; i < 10; i++)
        {
